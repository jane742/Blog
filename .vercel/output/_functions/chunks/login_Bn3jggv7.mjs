...
const $$Login = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, {}, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="container-fluid min-vh-100 d-flex align-items-center justify-content-center bg-light-subtle py-5"> <div class="row w-100 justify-content-center"> <div class="col-10 col-sm-8 col-md-6 col-lg-4"> <div class="card border-0 shadow-lg rounded-4 overflow-hidden"> <div class="bg-primary p-4 text-center text-white"> <div class="fs-1 mb-2">👋</div> <h1 class="h4 fw-bold mb-1">Вхід в систему</h1> <p class="small text-white-50 mb-0">
...
Ім'я користувача
</label> <div class="input-group shadow-inner rounded-3 overflow-hidden"> <span class="input-group-text bg-white border-end-0 text-muted">👤</span> <input type="text" name="username" id="username" class="form-control border-start-0 ps-0 py-2.5 custom-input" placeholder="Введіть ваш нікнейм" required autocomplete="username"> </div> </div> <div class="mb-4"> <label for="password" class="form-label fw-semibold text-secondary small">
Пароль
</label> <div class="input-group shadow-inner rounded-3 overflow-hidden"> <button type="button" id="togglePassword" class="input-group-text bg-white border-end-0 text-muted px-3" title="Показати/приховати пароль" style="cursor: pointer; border-right: 0;">
👁️
</button> <input type="password" name="password" id="password" class="form-control border-start-0 ps-2 py-2.5 custom-input" placeholder="Введіть ваш пароль" required autocomplete="current-password"> </div> </div> <button type="submit" class="btn btn-primary w-100 py-2.5 rounded-3 fw-bold shadow-sm transition-all mb-4">
Увйти →
...
або
</span> </div> <div class="text-center"> <p class="text-muted small mb-0">
Немає акаунта?
<a href="/signup" class="text-decoration-none fw-medium text-primary hover-underline ms-1">
Зареєструватися
</a> </p> </div> </div> </div> </div> </div> </div> ${renderScript($$result2, "C:/Users/jane/blog/src/pages/login.astro?astro&type=script&index=0&lang.ts")} ` })}`;
}, "C:/Users/jane/blog/src/pages/login.astro", void 0);

const $$file = "C:/Users/jane/blog/src/pages/login.astro";
const $$url = "/login";
